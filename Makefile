...
help:
	...
	@echo " seedusers      Seed predefined roles and users"
...
# Seed predefined roles and users (idempotent)
.PHONY: seedusers
seedusers:
	$(DC) exec app python manage.py seed_users
...
# Create a new Django app inside the app/ directory
# Usage: make app APP_NAME=<app_name>
.PHONY: app
app:
	@test -n "$(APP_NAME)" || (echo "Usage: make app APP_NAME=<app_name>" && exit 1)
	mkdir -p app/$(APP_NAME)
	python manage.py startapp $(APP_NAME) app/$(APP_NAME)
	@echo "Remember to add the app to the ./settings/base.py INSTALLED_APPS list."
